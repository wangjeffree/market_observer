...
class PumpFunScraper:
    ...
    def __init__(self, interval_minutes=5, end_time=None):
        """
        初始化爬虫
        ...
        if end_time:
            if isinstance(end_time, str):
                ...
                print(f"设置结束时间为: {self.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
            else:
                # 如果是datetime对象，直接使用
                self.end_time = end_time
                print(f"使用提供的datetime对象作为结束时间: {self.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def extract_coins_from_carousel(self, soup):
        ...
        if not coin_links:
            print("在CoinCarousel中未找到币种链接")
            return coins_data
            
        print(f"在CoinCarousel中找到币种链接: {len(coin_links)}个")
        ...
        for link in coin_links:
            try:
                ...
                print(f"提取币种: {name}, 市值: {market_value}, 回复数: {reply_count}, CA地址: {ca_address}")
            except Exception as e:
                print(f"处理币种链接时出错: {str(e)}")
    ...
    def fetch_and_save(self, output_filename=None, save_html=True):
        ...
        """
        print("开始一次性爬取网页并生成提取币种的文件...")
        
        try:
            html_content = None
            html_filename = None
            
            print("使用Selenium进行爬取...")
            # 设置Chrome选项
            ...
            # 从CoinCarousel部分提取
            print("尝试从CoinCarousel部分提取币种信息...")
            carousel_coins = self.extract_coins_from_carousel(soup)
            if carousel_coins:
                print(f"从CoinCarousel成功提取了{len(carousel_coins)}个币种信息")
                coins_data = carousel_coins
            else:
                # 查找所有币种链接
                print("从CoinCarousel提取失败，尝试查找所有币种链接...")
                coin_links = soup.find_all('a', href=lambda href: href and '/coin/' in href)
                if coin_links:
                    print(f"找到币种链接: {len(coin_links)}个")
                    temp_data = []
            ...
            if coins_data:
                ...
                print(f"成功获取并保存了{len(coins_data)}个trending币种数据到 {csv_file_path}")
                ...
                print("未能从网页中提取到trending币种数据")
                return False, [], None
                
        except Exception as e:
            print(f"爬取和保存数据时出错: {str(e)}")
            import traceback
            traceback.print_exc()
            return False, [], None
    ...
    def compare_and_update_history(self, new_coins_data):
        ...
        # 提取新数据中的symbol列表，确保只处理非空字符串
        new_symbols = [coin['symbol'].lower() for coin in new_coins_data if coin['symbol'] and isinstance(coin['symbol'], str)]
        
        # 检查历史数据文件是否存在
        if not os.path.exists(self.history_file):
            print(f"历史数据文件不存在，创建新文件: {self.history_file}")
            # 创建历史数据文件
        ...
        try:
            ...
            # 提取历史数据中的symbol列表，确保只处理非空字符串
            history_symbols = []
            for symbol in history_df['symbol'].tolist():
                # 检查symbol是否为有效字符串
                if isinstance(symbol, str) and symbol.strip():
                    history_symbols.append(symbol.lower())
                elif not pd.isna(symbol) and symbol is not None:
                    # 如果不是NaN但也不是字符串，尝试转换为字符串
                    try:
                        symbol_str = str(symbol).strip()
                        if symbol_str:
                            history_symbols.append(symbol_str.lower())
                    except:
                        pass
            ...
            if new_added_symbols:
                print(f"发现新增币种: {', '.join(new_added_symbols)}")
                # 更新历史数据（追加新币种）
                new_coins_to_add = [coin for coin in new_coins_data if coin['symbol'] and isinstance(coin['symbol'], str) and coin['symbol'].lower() in new_added_symbols]
                ...
                print(f"已将新币种添加到历史数据文件: {self.history_file}")
                
                # 发送新币种通知
                self.send_notification("new_coins", new_coins_to_add)
                
            elif removed_symbols:
                print(f"发现已移除币种: {', '.join(removed_symbols)}")
                # 更新历史数据（替换为新数据）
                self.save_to_csv(new_coins_data, os.path.basename(self.history_file))
                print(f"已用新数据替换历史数据文件: {self.history_file}")
                ...
                print("币种列表未发生变化，无需更新历史数据")
                
        except Exception as e:
            print(f"比对和更新历史数据时出错: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def run(self, output_filename=None, save_html=True):
        ...
        try:
            ...
            if success:
                print(f"爬虫运行成功，共获取了{len(coins_data)}个trending币种数据")
                print(f"数据已保存到: {csv_file_path}")
                return True
            else:
                print("爬虫运行失败，未能获取trending币种数据")
                return False
    ...
    def test_extract_from_html_file(self, html_file_path=None):
        ...
        try:
            ...
            print(f"成功读取HTML文件: {html_file_path}")
            soup = BeautifulSoup(html_content, 'html.parser')
            ...
            if coins_data:
                print(f"成功从HTML文件中提取了{len(coins_data)}个币种信息")
                # 保存到CSV
                ...
                print("未能从HTML文件中提取币种信息")
                return []
                
        except Exception as e:
            print(f"测试提取币种信息时出错: {str(e)}")
            return []
    ...
    def run_scheduled(self):
        ...
        print(f"开始定时运行PumpFun爬虫，间隔时间：{self.interval_minutes}分钟")
        
        if self.end_time:
            print(f"结束时间设置为：{self.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
        else:
            print("未设置结束时间，将一直运行直到手动停止")
        ...
        try:
            ...
            while self.is_running:
                ...
                if self.end_time and current_time >= self.end_time:
                    print(f"当前时间 {current_time.strftime('%Y-%m-%d %H:%M:%S')} 已超过结束时间 {self.end_time.strftime('%Y-%m-%d %H:%M:%S')}，停止运行")
                    break
                ...
                if self.end_time and next_run_time >= self.end_time:
                    print(f"下次运行时间 {next_run_time.strftime('%Y-%m-%d %H:%M:%S')} 将超过结束时间 {self.end_time.strftime('%Y-%m-%d %H:%M:%S')}，停止运行")
                    break
                ...
                if wait_seconds > 0:
                    print(f"等待 {wait_seconds:.1f} 秒后进行下一次运行...")
                    # 每10秒检查一次是否应该停止
